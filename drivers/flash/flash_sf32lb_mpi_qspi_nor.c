...
static __ramfunc void qspi_nor_cinstr(const struct device *dev, uint8_t cmd)
{
	...
	sys_write32(ccr1, config->mpi + MPI_CCR1);

	/* send command and wait for completion */
	sys_write32(cmd, config->mpi + MPI_CMDR1);

	while (!sys_test_bit(config->mpi + MPI_SR, MPI_SR_TCF_Pos))
		;

	sys_write32(MPI_SCR_TCFC, config->mpi + MPI_SCR);
}
						     ...
						     uint32_t ccrx, uint32_t addr)
{
	...
	sys_write32(MPI_CCRX_CMD_RDSR, config->mpi + MPI_CCR2);
	sys_write32(SPI_NOR_CMD_RDSR, config->mpi + MPI_CMDR2);
	sys_write32(0U, config->mpi + MPI_DLR2);
	sys_write32(SPI_NOR_MEM_RDY_MASK, config->mpi + MPI_SMKR);
	sys_write32(SPI_NOR_MEM_RDY_MATCH, config->mpi + MPI_SMR);

	cr = sys_read32(config->mpi + MPI_CR);
	cr |= MPI_CR_CMD2E | MPI_CR_SME2;
	sys_write32(cr, config->mpi + MPI_CR);

	/* issue CMD1, wait for status match */
	sys_write32(addr, config->mpi + MPI_AR1);

	sys_write32(ccrx, config->mpi + MPI_CCR1);
	sys_write32(cmd, config->mpi + MPI_CMDR1);
	while (!sys_test_bit(config->mpi + MPI_SR, MPI_SR_SMF_Pos))
		;

	sys_write32(MPI_SCR_SMFC | MPI_SCR_TCFC, config->mpi + MPI_SCR);
					 ...
					 uint32_t addr, void *buf, size_t len)
{
	...
	do {
		...
		sys_write32(FIELD_PREP(MPI_DLR1_DLEN_Msk, chunk_len - 1U), config->mpi + MPI_DLR1);
		sys_write32(addr, config->mpi + MPI_AR1);
		...
		for (size_t i = 0U; i < (chunk_len / 4U); i++) {
			uint32_t data = sys_read32(config->mpi + MPI_DR);
			memcpy(&cbuf[i * 4U], &data, 4U);
		}

		if (chunk_len & 3U) {
			uint32_t data = sys_read32(config->mpi + MPI_DR);
			memcpy(&cbuf[chunk_len & ~3U], &data, chunk_len & 3U);
		}
					  ...
					  uint32_t addr, void *buf, size_t len)
{
	...
	do {
		...
		for (size_t i = 0U; i < (chunk_len / 4U); i++) {
			uint32_t data;
			memcpy(&data, &cbuf[i * 4U], 4U);
			sys_write32(data, config->mpi + MPI_DR);
		}

		if (chunk_len & 3U) {
			uint32_t data;
			memcpy(&data, &cbuf[chunk_len & ~3U], chunk_len & 3U);
			sys_write32(data, config->mpi + MPI_DR);
		}
...
static int flash_sf32lb_mpi_qspi_nor_read(const struct device *dev, off_t offset, void *data,
					  size_t size)
{
	const struct flash_sf32lb_mpi_qspi_nor_config *config = dev->config;
	...
	memcpy(data, (void *)(config->base + offset), size);
						     ...
						     const void *data, size_t size)
{
	...
	while (size > 0) {
		...
		/* limit to one page size, without crossing page boundary */
		...
		(void)sf32lb_dma_reload_dt(&config->dma, (uintptr_t)cdata, config->mpi + MPI_DR,
					   chunk_len);
		(void)sf32lb_dma_start_dt(&config->dma);

		/* enable write, send command and wait until ready */
		qspi_nor_cinstr(dev, SPI_NOR_CMD_WREN);
		qspi_nor_cinstr_seq_ready_wait(dev, config->cmd_pp, config->ccrx_pp, offset);

		/* wait for DMA completion (polling) */
		do {
			ret = sf32lb_dma_get_status_dt(&config->dma, &status);
		...
		sys_cache_data_invd_range((void *)(config->base + offset), chunk_len);
						     ...
						     size_t size)
{
	...
	do {
		...
		sys_cache_data_invd_range((void *)(config->base + offset), size);
...
static __ramfunc int flash_sf32lb_mpi_qspi_nor_init(const struct device *dev)
{
	...
	if (!sf32lb_dma_is_ready_dt(&config->dma)) {
		return -ENODEV;
	}

	sys_write32(0xFF, config->mpi + MPI_TIMR);
	sys_write32(0x50005000, config->mpi + MPI_CIR);
	sys_write32(0xFF, config->mpi + MPI_ABR1);
	sys_write32(0xff, config->mpi + MPI_HRABR);

	val = sys_read32(config->mpi + MPI_FIFOCR);
	val &= MPI_FIFOCR_TXSLOTS_Msk;
	val |= FIELD_PREP(MPI_FIFOCR_TXSLOTS_Msk, 1U);
	sys_write32(val, config->mpi + MPI_FIFOCR);

	val = sys_read32(config->mpi + MPI_MISCR);
	val &= ~MPI_MISCR_RXCLKINV_Msk;
	val |= FIELD_PREP(MPI_MISCR_RXCLKINV_Msk, !!config->invert_rx_clk);
	sys_write32(val, config->mpi + MPI_MISCR);

	sys_write32(config->psclr, config->mpi + MPI_PSCLR);

	/* enable QSPI (non-dual mode) */
	val = sys_read32(config->mpi + MPI_CR);
	val &= ~MPI_CR_DFM;
	val |= MPI_CR_EN;
	sys_write32(val, config->mpi + MPI_CR);

	if (config->addr_len == 4U) {
		/* enable 4-byte address mode */
		qspi_nor_cinstr(dev, SPI_NOR_CMD_4BA);

		/* configure row boundary size */
		val = sys_read32(config->mpi + MPI_DCR);
		val &= ~MPI_DCR_RBSIZE_Msk;
		val |= FIELD_PREP(MPI_DCR_RBSIZE_Msk, config->rb_size);
		sys_write32(val, config->mpi + MPI_DCR);
	}

	/* enable QER */
	if (config->qer != JESD216_DW15_QER_VAL_NONE) {
		uint8_t sr[2];
	...
	sys_write32(config->ccrx_read, config->mpi + MPI_HRCCR);

	val = sys_read32(config->mpi + MPI_HCMDR);
	val &= ~MPI_HCMDR_RCMD_Msk;
	val |= FIELD_PREP(MPI_HCMDR_RCMD_Msk, config->cmd_read);
	sys_write32(val, config->mpi + MPI_HCMDR);

	/* perform initial DMA configuration (so we can just reload) */
	sf32lb_dma_config_init_dt(&config->dma, &config_dma);
	...
	ret = sf32lb_dma_config_dt(&config->dma, &config_dma);
	if (ret < 0) {
		return ret;
	}
		...
		.layout =                                                                          \
			{                                                                          \
				...
				.pages_size = SPI_NOR_PAGE_SIZE,                                   \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, flash_sf32lb_mpi_qspi_nor_init, NULL, NULL, &config##n,           \
